    ...
    savePlaying: function() {
      ...
      for (var i = _players.length - 1; i >= 0; i--) {
        if (_players[i]._a && !_players[i]._a.paused) {
          ...
          // parked player has no .message-row ancestor, and overwriting _msgId
          // with null there is what used to lose the association for good: the
          // player could then never find its row again and stayed a stray block
          // in the feed for the rest of its life.
