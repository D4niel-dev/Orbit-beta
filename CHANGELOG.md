...
- **Every Empty State Uses One Treatment** — The app had five "nothing here yet" surfaces and they had drifted apart: a bare icon and a grey line in the chat panel, a plain sentence in the sidebar, a bolded query in search. They are the same idea, so they are one component now (`shared/ui/empty-state.js`): a glass icon tile with a soft bloom, a title, an optional hint, an optional action — with a compact scale for lists, and a muted tone for passive states, because "waiting for peers" is not something to act on and should not wear the accent. The chat panel's state names the friend you are about to message; search keeps your query in the hint.
