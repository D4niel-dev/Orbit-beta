...
- **Android Notifications Carry the Sender's Face** — The small icon is the Orbit mark, so the status bar says which app it came from; the notification's large icon is now the sender's avatar, so the shade says who. That is the pairing Discord uses, and it costs nothing when the sender has no avatar — the lookup returns null and the large icon is simply left off. The native decode is defensive: anything that is not a decodable image data URL is dropped rather than throwing inside the notification path.
