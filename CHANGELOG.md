...
- **Messages to an Offline Peer Now Wait** — Orbit had no send queue at all: every send was fire-and-forget, so a message to a peer who was not currently connected was simply gone. On a LAN that is constant — a laptop lid closes, a phone leaves Wi-Fi range — and it was silently dropping messages. A new outbox holds the packet and delivers it the moment the peer is reachable, on app foreground or when they reappear. The queue lives in localStorage so it survives the app being killed, is capped at 500 entries, and gives up after a week rather than retrying forever. Queued bubbles show a quiet hourglass so a message never looks delivered when it is not. Typing, presence, reactions and call signalling are deliberately **not** queued — a typing indicator five minutes late is worse than none, and a stale call offer is actively harmful.
