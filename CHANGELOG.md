...
- **The All Friends Tab** — The Friends tab only lists who is online, and a closed DM is hidden from it entirely, so there was no way to start a conversation with an offline friend *or* to recover a DM closed by accident. The new tab lists every friend — Echo first, then online, then offline, closed ones marked "Closed — click to reopen" — and clicking a closed row reopens it. It still respects the per-account chat filter, so one account's friends cannot leak into another's sidebar.
